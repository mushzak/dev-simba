...
(function($) {
    $.fn.formToWizard = function( options, cmdParam1 ) {
        ...
        var element = this
            ...
            , count = steps.size()
            , beforeButtonName = "#" + options.beforeButton
        ...
        if( typeof options !== 'string' ) {
            ...
            steps.each(function(i) {
                ...
                .after('<p id="step' + i + 'commands" class="commands"></p>');
                ...
                if (i == 0) {
                    createNextButton(i);
                    selectStep(i);
                    // move before button to the first step
                    $(beforeButtonName).addClass('prev').detach().appendTo("#step" + i + "commands");
                }
                else if (i == count - 1) {
                    $("#step" + i).hide();
                    createPrevButton(i);
        ...
        function selectStep(i) {
            ...
            if( typeof(options.select) === 'function' ) {
                options.select(element, step);
            }

            element.trigger('select.wizard', [element, step]);
        }
        /******************** End Private Methods ********************/
